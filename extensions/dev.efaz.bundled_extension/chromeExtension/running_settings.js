...
(() => {
    ...
    async function saveData() {
        storage.get([system_settings.name], async (items) => {
            ...
            storage.set(items, () => {
                alert(chrome.i18n.getMessage("settingsSavedData") || "Saved data!");
                changes_made = {};
            });
        });
    }
    ...
    async function loadChanges() {
        await localizeAll();
        try {
            const setting_res = await fetch(chrome.runtime.getURL(`${document.extensionName}/settings.json`));
            system_settings = await setting_res.json();
        } catch (e) {
            console.error(`Failed to load ${document.extensionName}/settings.json`, e);
            return;
        }
            ...
            if (settingsKeys.length === 1 && document.getElementById("extensionSettings")) {
                ...
                for (const [key, val] of settingsKeys) {
                    ...
                    main_selection.addEventListener("change", () => {
                        if (val.type === "checkbox") {
                            changes_made[key] = main_selection.checked !== selected;
                        } else if (val.type === "file") {
                            changes_made[key] = !main_selection.getAttribute("file_url");
                        } else {
                            changes_made[key] = main_selection.value !== selected;
                        }
                    ...
                    if (val.alternateFileInput && val.type === "file") {
                        ...
                        if (button) {
                            ...
                            main_selection.addEventListener("change", () => {
                                button.textContent = main_selection.files.length > 0 ? main_selection.files[0].name : (storedConfig[`${key}_filename`] || "No file selected");
                            });
                    ...
                    if (val.reset) {
                        ...
                        if (resetBtn) {
                            resetBtn.addEventListener("click", () => {
                                ...
                                changes_made[key] = (val.type === "checkbox") ? main_selection.checked !== selected : main_selection.value !== selected;
                            });
                        }
                    }
                }
        ...
        try {
            const man_res = await fetch(chrome.runtime.getURL(`${document.extensionName}/org_manifest.json`));
            const man_json = await man_res.json();
            const replacer = (_, v1) => v1 ? chrome.i18n.getMessage(v1) : "";
            const extension_name = (man_json.name || "").replace(/__MSG_(\w+)__/g, replacer);
            ...
            if (extensNameEl) extensNameEl.innerHTML = `${getTran("settingsExName")} ${extension_name} <img src="${document.extensionName}/${extension_icon}" height="16" width="16" style="vertical-align: middle;">`;
            ...
            if (extLinkEl) {
                ...
                if (isChrome && system_settings.chromeWebstoreLinkEnabled) {
                    const idToUse = (chrome.runtime.id === system_settings.uploadedChromeExtensionID) 
                        ? chrome.runtime.id : system_settings.uploadedChromeExtensionID;
                    ...
                    const idToUse = (chrome.runtime.id === system_settings.uploadedFirefoxExtensionID) 
                        ? chrome.runtime.id : system_settings.uploadedFirefoxExtensionID;
                ...
                if (r.ok) {
                    ...
                    if (compared === -1 && extensVersEl) {
                        ...
                        document.getElementById("openChromeExtensionSettings").addEventListener("click", () => {
                            ...
                            if (mode === "chrome") {
                                const url = (system_settings.chromeWebstoreLinkEnabled && chrome.runtime.id !== system_settings.uploadedChromeExtensionID)
                                    ? `https://chromewebstore.google.com/detail/extension/${system_settings.uploadedChromeExtensionID}`
                                    : "chrome://extensions/";
                                chrome.tabs.create({ url });
                            } else if (mode === "firefox") {
                                const url = (system_settings.firefoxWebstoreLinkEnabled && chrome.runtime.id !== system_settings.uploadedFirefoxExtensionID)
                                    ? `https://addons.mozilla.org/en-US/firefox/addon/${system_settings.uploadedFirefoxExtensionID}`
            ...
            console.error("Failed handling org_manifest.json or update logic", e);
        }
    }
