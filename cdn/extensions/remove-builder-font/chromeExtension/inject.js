...
(function () { // Prevent changes made using the Inspect console.
    ...
    function overwriteResourcesUrl(made_css, source, subdomain_type, old_font_on_sub) {
            ...
            if (enabled == true) {
                ...
                if (tab.href) {
                    ...
                    if (urlObj.hostname == "www.roblox.com") {
                        ...
                        if (stored_css) {
                            injectCSS(overwriteResourcesUrl(stored_css, trusted_source, 1, oldFontOnOtherSub))
                        } else {
                            if (remoteStyles == true) {
                                fetch("https://cdn.efaz.dev/cdn/extensions/remove-builder-font/chromeExtension/change_font.css").then(res => { return res.text() }).then(fetched => {
                                    stored_css = fetched
                                    injectCSS(overwriteResourcesUrl(fetched, trusted_source, 1, oldFontOnOtherSub))
                                })
                            } else {
                        ...
                        if (devForum == true) {
                            ...
                            if (stored_devforum_css) {
                                injectCSS(overwriteResourcesUrl(stored_devforum_css, trusted_source, 2, oldFontOnOtherSub))
                            } else {
                                if (remoteStyles == true) {
                                    fetch("https://cdn.efaz.dev/cdn/extensions/remove-builder-font/chromeExtension/devforum_font.css").then(res => { return res.text() }).then(fetched => {
                                        ...
                                        injectCSS(overwriteResourcesUrl(fetched, trusted_source, 2, oldFontOnOtherSub))
                                    })
                                } else {
                        ...
                        if (overwriteCreateDashboard == true) {
                            ...
                            if (stored_creator_dashboard_css) {
                                injectCSS(overwriteResourcesUrl(stored_creator_dashboard_css, trusted_source, 3, oldFontOnOtherSub))
                            } else {
                                if (remoteStyles == true) {
                                    fetch("https://cdn.efaz.dev/cdn/extensions/remove-builder-font/chromeExtension/global_font.css").then(res => { return res.text() }).then(fetched => {
                                        ...
                                        injectCSS(overwriteResourcesUrl(fetched, trusted_source, 3, oldFontOnOtherSub))
                                    })
                                } else {
                        ...
                        if (otherSub == true && !(urlObj.hostname.includes("create.roblox.com"))) {
                            ...
                            if (stored_creator_dashboard_css) {
                                injectCSS(overwriteResourcesUrl(stored_creator_dashboard_css, trusted_source, 4, oldFontOnOtherSub))
                            } else {
                                if (remoteStyles == true) {
                                    fetch("https://cdn.efaz.dev/cdn/extensions/remove-builder-font/chromeExtension/global_font.css").then(res => { return res.text() }).then(fetched => {
                                        stored_creator_dashboard_css = fetched
                                        injectCSS(overwriteResourcesUrl(fetched, trusted_source, 4, oldFontOnOtherSub))
                                    })
                                } else {
                                    fetch(chrome.runtime.getURL("global_font.css")).then(res => { return res.text() }).then(fetched => {
                                        stored_creator_dashboard_css = fetched
